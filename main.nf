...
process total_spectrum{
 ...
 input:
  set sample_id, path(ibd), path(imzml) from ch_pairs
 ...
 output:
  ...
  echo $imzml > file
  echo $ibd > file
  matlab -nodesktop -nodisplay -r "total_spectrum('$imzml', '$sap', '$sa_path');exit"
  """
}
...
process peak_picking{
 ...
 """
 matlab -nodesktop -nodisplay -r "peak_picking('$input_file', $nzm_multiple, '$sa_path);exit"
 """
...
process make_datacube {
 ...
 input:
  ...
  """
  matlab -nodesktop -nodisplay -r "make_datacube('$imzml', '$peaks', '$sa_path');exit"
  """
}
